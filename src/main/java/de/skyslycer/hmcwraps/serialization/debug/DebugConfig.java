...
public class DebugConfig extends Config implements Debuggable {
    ...
    public DebugConfig(Config config) {
        super((UpdaterSettings) config.getUpdater(), (PermissionSettings) config.getPermissions(), (PreviewSettings) config.getPreview(),
                (Toggleable) config.getFavorites(), (Inventory) config.getInventory(), (SerializableItem) config.getUnwrapper(),
                (PreservationSettings) config.getPreservation(), null, null, (FilterSettings) config.getFilter());
    }
