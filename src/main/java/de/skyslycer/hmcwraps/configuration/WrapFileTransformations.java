...
public class WrapFileTransformations {

    private static final int LATEST_VERSION = 0;

    private WrapFileTransformations() { }
    ...
    public static <N extends ConfigurationNode> N updateNode(final N node) throws ConfigurateException {
        if (!node.virtual()) {
            ...
            if (startVersion != endVersion) {
                Bukkit.getLogger().info("Updated wrap file config schema from " + startVersion + " to " + endVersion);
            }
        }
        return node;
    }
